...
if [ $(id -u) == 0 ] ; then
    ...
    # Only attempt to change the aleksey username if it exists
    if id aleksey &> /dev/null ; then
        echo "Set username to: $NB_USER"
        usermod -d /home/$NB_USER -l $NB_USER aleksey
    fi
    ...
    if [[ "$NB_USER" != "aleksey" ]]; then
        # changing username, make sure homedir exists
        # (it could be mounted, and we shouldn't create it if it already exists)
        if [[ ! -e "/home/$NB_USER" ]]; then
            echo "Relocating home dir to /home/$NB_USER"
            mv /home/aleksey "/home/$NB_USER" || ln -s /home/aleksey "/home/$NB_USER"
        fi
        # if workdir is in /home/aleksey, cd to /home/$NB_USER
        if [[ "$PWD/" == "/home/aleksey/"* ]]; then
    ...
    if [[ "$NB_UID" == "$(id -u aleksey)" && "$NB_GID" == "$(id -g aleksey)" ]]; then
        ...
        if [[ "$STATUS" != "0" ]]; then
            if [[ -w /etc/passwd ]]; then
                ...
                cat /etc/passwd | sed -e "s/^aleksey:/nayvoj:/" > /tmp/passwd
                echo "aleksey:x:$(id -u):$(id -g):,,,:/home/aleksey:/bin/bash" >> /tmp/passwd
